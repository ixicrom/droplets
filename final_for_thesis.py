...
if input('Make t-SNE graphs? y/n: ') == 'y':
    ...
    for p in perp_vals:
        ...
        pl.scatter(dat_tsne_test[:, 0], dat_tsne_test[:, 1])
        pl.title('t-SNE '+str(data_tag)+' data, random state=10, p=' +str(p)+' '+file_suffix.replace('_', ', '))
...
if input('Plot example images and clusters? y/n: ') == 'y':
    ...
    for item in image_list:
        ...
        pl.tight_layout()
        pl.savefig(graph_folder+'hbar_phi'+file_suffix+item.replace('\_', '_')+'.png', transparent=True)
        pl.show()
